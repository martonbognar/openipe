...
// Read/Write probes
wire              reg_write =  |per_we & (reg_sel | reg_sel2 | reg_sel3);
wire              reg_read  = ~|per_we & (reg_sel | reg_sel2 | reg_sel3);
...
wire [2:0] ipe_enabled;
wire [2:0] ipe_puc_on_violation;
wire [2:0] ipe_locked;
`ifdef IPE_GRAN
  wire [2:0] ipe_granularity;
  wire [2:0] shift_amt [2:0];
`endif
`ifdef IPE_PC
  wire [2:0] ipe_child;
  wire [2:0] ipe1_parent;
  wire [2:0] ipe2_parent;
  wire [2:0] ipe3_parent;
`endif
...
`ifdef IPE_GRAN
  ...
  assign ipe_seg_end = ((MPUIP3SEGB2 << shift_amt[2]) & {16{ipe_executing[2]}}) | 
                       ((MPUIP2SEGB2 << shift_amt[1]) & {16{ipe_executing[1]}}) | 
...
`else
  ...
  assign ipe_seg_end = (MPUIP3SEGB2 << 4 & {16{ipe_executing[2]}}) | 
                       (MPUIP2SEGB2 << 4 & {16{ipe_executing[1]}}) | 
...
`ifdef IPE_GRAN
  function automatic [2:0] address_in_ipe (input [15:0] addr);
  begin
    address_in_ipe = {((addr >> shift_amt[2]) < MPUIP3SEGB2 & (addr >> shift_amt[2]) >= MPUIP3SEGB1),
...
`else
  function automatic [2:0] address_in_ipe (input [15:0] addr);
  begin
    address_in_ipe = {(addr >> 4) < MPUIP3SEGB2 & (addr >> 4) >= MPUIP3SEGB1, (addr >> 4) < MPUIP2SEGB2 & (addr >> 4) >= MPUIP2SEGB1 , (addr >> 4) < MPUIPSEGB2 & (addr >> 4) >= MPUIPSEGB1};
  end
  endfunction
`endif
...
`ifdef IPE_PC
  function automatic [2:0] ipe_parent (input [2:0] ipe_id);
  begin
    ipe_parent = (ipe1_parent & {4{ipe_id[0]}}) | (ipe2_parent & {4{ipe_id[1]}}) | (ipe3_parent & {4{ipe_id[2]}}) ;
  end
  endfunction
`endif

`ifndef OMIT_IPE_FIXES
  wire      [2:0] pc_in_ipe = address_in_ipe (buff_pc);
`else
  wire      [2:0] pc_in_ipe = address_in_ipe (fe_pc);
`endif

wire    [2:0] eu_mem_in_ipe = address_in_ipe (eu_mab);
wire       [2:0] dma_in_ipe = address_in_ipe (dma_addr);
wire       [2:0] dbg_in_ipe = address_in_ipe (dbg_mem_addr);
...
`ifndef OMIT_IPE_FIXES
  ...
  `endif
  wire       [2:0] fe_pc_in_ipe = address_in_ipe (fe_pc);
  wire   [2:0] fe_pc_nxt_in_ipe = address_in_ipe (fe_pc_nxt);
  wire    [2:0] fe_nxt_in_entry = {(fe_pc_nxt == entry_point3) ,(fe_pc_nxt == entry_point2), (fe_pc_nxt == entry_point)};
...
assign ipe_exec_three = ipe_enabled & pc_in_ipe;
assign ipe_executing = {1'b0, ipe_exec_three};
...
always @(posedge mclk or posedge puc_rst)
  ...
  else if (|pc_in_ipe) IPE_ACTIVE[2:0] <= pc_in_ipe;
...
assign per_dout        =  (ipc0_rd | ip2c0_rd | ip3c0_rd) |
                          (segb2_rd | seg2b2_rd | seg3b2_rd) |
                          (segb1_rd | seg2b1_rd | seg3b1_rd) |
                          active_rd;
