...
`ifdef IPE_GRAN
  wire ipe_granularity = MPUIPC0[8];
  wire [2:0] shift_amt = ipe_granularity ? 3'd0 : 3'd4;
`endif
...
`ifdef IPE_GRAN
  ...
  assign ipe_seg_end = MPUIPSEGB2 << shift_amt;
`else
  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIPSEGB2 <=  16'h0000;
...
`ifdef IPE_GRAN
  ...
  begin
    address_in_ipe = {((addr >> shift_amt) < MPUIPSEGB2 & (addr >> shift_amt) >= MPUIPSEGB1)};   
  end
  endfunction
`else
  function automatic address_in_ipe (input [15:0] addr);
...
`ifndef OMIT_IPE_FIXES
  `ifdef IPE_GRAN
    wire [15:0] entry_point = (MPUIPSEGB1 << shift_amt) + 8;
  `else
...
assign ipe_exec_single = ipe_enabled & pc_in_ipe;
assign ipe_executing = {3'b000, ipe_exec_single};
