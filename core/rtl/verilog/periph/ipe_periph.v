...
parameter       [14:0] BASE_ADDR3   = 15'h05B8;
parameter       [14:0] BASE_ADDR4   = 15'h05C0;
...
wire 		  reg_sel3  =  per_en & (per_addr[13:DEC_WD-1]==BASE_ADDR3[14:DEC_WD]);
wire 		  reg_sel4  =  per_en & (per_addr[13:DEC_WD-1]==BASE_ADDR4[14:DEC_WD]);
...
// Read/Write probes
wire              reg_write =  |per_we & (reg_sel | reg_sel2 | reg_sel3 | reg_sel4);
wire              reg_read  = ~|per_we & (reg_sel | reg_sel2 | reg_sel3 | reg_sel4);
...
wire [3:0] ipe_enabled;
wire [3:0] ipe_puc_on_violation;
wire [3:0] ipe_locked;
`ifdef IPE_GRAN
  wire [3:0] ipe_granularity;
  wire [2:0] shift_amt [3:0];
`endif
`ifdef IPE_PC
  wire [3:0] ipe_child;
  wire [3:0] ipe1_parent;
  wire [3:0] ipe2_parent;
  wire [3:0] ipe3_parent;
  wire [3:0] ipe4_parent;
`endif
...
`ifdef IPE_GRAN
  assign ipe_granularity[1] = MPUIP2C0[8];
`endif
`ifdef IPE_PC
...
assign ipe_puc_on_violation[2] = MPUIP3C0[5];  // TODO: use this
assign ipe_enabled[2] = MPUIP3C0[6];
assign ipe_locked[2] = MPUIP3C0[7];
`ifdef IPE_GRAN
  assign ipe_granularity[2] = MPUIP3C0[8];
`endif
`ifdef IPE_PC
  assign ipe_child[2] = MPUIP3C0[9];
  assign ipe3_parent     = MPUIP3C0[9] ? 1 << (MPUIP3C0[12:10] - 1) : 4'b0;
`endif

wire ip3c0_wr = reg_wr[IPC0_OFFSET] & (~ipe_locked[2]) & reg_sel3;

always @ (posedge mclk or posedge puc_rst)
  if (puc_rst)      MPUIP3C0 <=  16'h0000;
  else if (ip3c0_wr) MPUIP3C0 <=  per_din;

reg  [15:0] MPUIP4C0;

assign ipe_puc_on_violation[3] = MPUIP4C0[5];  // TODO: use this
assign ipe_enabled[3] = MPUIP4C0[6];
assign ipe_locked[3] = MPUIP4C0[7];
`ifdef IPE_GRAN
  assign ipe_granularity[3] = MPUIP4C0[8];
`endif
`ifdef IPE_PC
  assign ipe_child[3] = MPUIP4C0[9];
  assign ipe4_parent     = MPUIP4C0[9] ? 1 << (MPUIP4C0[12:10] - 1) : 4'b0;
`endif

wire ip4c0_wr = reg_wr[IPC0_OFFSET] & (~ipe_locked[3]) & reg_sel4;

always @ (posedge mclk or posedge puc_rst)
  if (puc_rst)      MPUIP4C0 <=  16'h0000;
  else if (ip4c0_wr) MPUIP4C0 <=  per_din;

`ifdef IPE_GRAN
  assign shift_amt[0] = ipe_granularity[0] ? 3'd0 : 3'd4;
  assign shift_amt[1] = ipe_granularity[1] ? 3'd0 : 3'd4;
  assign shift_amt[2] = ipe_granularity[2] ? 3'd0 : 3'd4;
  assign shift_amt[3] = ipe_granularity[3] ? 3'd0 : 3'd4;
`endif
...
`ifdef IPE_GRAN
  ...
  reg  [15:0] MPUIP3SEGB2;
  reg  [15:0] MPUIP4SEGB2;
`else
  reg  [12:0] MPUIPSEGB2;
  reg  [12:0] MPUIP2SEGB2;
  reg  [12:0] MPUIP3SEGB2;
  reg  [12:0] MPUIP4SEGB2;
`endif

wire segb2_wr = reg_wr[IPSEGB2_OFFSET] & (~ipe_locked[0]) & reg_sel;
wire seg2b2_wr = reg_wr[IPSEGB2_OFFSET] & (~ipe_locked[1]) & reg_sel2;
wire seg3b2_wr = reg_wr[IPSEGB2_OFFSET] & (~ipe_locked[2]) & reg_sel3;
wire seg4b2_wr = reg_wr[IPSEGB2_OFFSET] & (~ipe_locked[3]) & reg_sel4;
...
`ifdef IPE_GRAN
  ...
  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIP3SEGB2 <=  16'h0000;
    else if (seg3b2_wr) MPUIP3SEGB2 <=  per_din;

  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIP4SEGB2 <=  16'h0000;
    else if (seg4b2_wr) MPUIP4SEGB2 <=  per_din;

  assign ipe_seg_end = ((MPUIP4SEGB2 << shift_amt[3]) & {16{ipe_executing[3]}}) | ((MPUIP3SEGB2 << shift_amt[2]) & {16{ipe_executing[2]}}) 
                      | ((MPUIP2SEGB2 << shift_amt[1]) & {16{ipe_executing[1]}}) | ((MPUIPSEGB2 << shift_amt[0]) & {16{ipe_executing[0]}});
...
`else
  ...
  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIP3SEGB2 <=  16'h0000;
    else if (seg3b2_wr) MPUIP3SEGB2 <=  per_din[12:0];

  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIP4SEGB2 <=  16'h0000;
    else if (seg4b2_wr) MPUIP4SEGB2 <=  per_din[12:0];

  assign ipe_seg_end = (MPUIP4SEGB2 << 4 & {16{ipe_executing[3]}}) | (MPUIP3SEGB2 << 4 & {16{ipe_executing[2]}}) 
                      | (MPUIP2SEGB2 << 4 & {16{ipe_executing[1]}}) | (MPUIPSEGB2 << 4 & {16{ipe_executing[0]}});
...
`ifdef IPE_GRAN
  ...
  reg  [15:0] MPUIP3SEGB1;
  reg  [15:0] MPUIP4SEGB1;
`else
  reg  [12:0] MPUIPSEGB1;
  reg  [12:0] MPUIP2SEGB1;
  reg  [12:0] MPUIP3SEGB1;
  reg  [12:0] MPUIP4SEGB1;
`endif

wire segb1_wr = reg_wr[IPSEGB1_OFFSET] & (~ipe_locked[0]) & reg_sel;
wire seg2b1_wr = reg_wr[IPSEGB1_OFFSET] & (~ipe_locked[1]) & reg_sel2;
wire seg3b1_wr = reg_wr[IPSEGB1_OFFSET] & (~ipe_locked[2]) & reg_sel3;
wire seg4b1_wr = reg_wr[IPSEGB1_OFFSET] & (~ipe_locked[3]) & reg_sel4;
...
`ifdef IPE_GRAN
  ...
  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIP3SEGB1 <=  16'h0000;
    else if (seg3b1_wr) MPUIP3SEGB1 <=  per_din;

  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIP4SEGB1 <=  16'h0000;
    else if (seg4b1_wr) MPUIP4SEGB1 <=  per_din;
`else
  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIPSEGB1 <=  16'h0000;
  ...
  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIP3SEGB1 <=  16'h0000;
    else if (seg3b1_wr) MPUIP3SEGB1 <=  per_din[12:0];

  always @ (posedge mclk or posedge puc_rst)
    if (puc_rst)       MPUIP4SEGB1 <=  16'h0000;
    else if (seg4b1_wr) MPUIP4SEGB1 <=  per_din[12:0];
`endif
...
`ifdef IPE_GRAN
  function automatic [3:0] address_in_ipe (input [15:0] addr);
  begin
    address_in_ipe = {((addr >> shift_amt[3]) < MPUIP4SEGB2 & (addr >> shift_amt[3]) >= MPUIP4SEGB1),
                      ((addr >> shift_amt[2]) < MPUIP3SEGB2 & (addr >> shift_amt[2]) >= MPUIP3SEGB1),
                      ((addr >> shift_amt[1]) < MPUIP2SEGB2 & (addr >> shift_amt[1]) >= MPUIP2SEGB1),
...
`else
  function automatic [3:0] address_in_ipe (input [15:0] addr);
  begin
    address_in_ipe = {((addr >> 4) < MPUIP4SEGB2 & (addr >> 4) >= MPUIP4SEGB1),((addr >> 4) < MPUIP3SEGB2 & (addr >> 4) >= MPUIP3SEGB1),
    ((addr >> 4) < MPUIP2SEGB2 & (addr >> 4) >= MPUIP2SEGB1) , ((addr >> 4) < MPUIPSEGB2 & (addr >> 4) >= MPUIPSEGB1)};
  end
  endfunction
`endif

`ifdef IPE_PC
  function automatic [3:0] ipe_parent (input [3:0] ipe_id);
  begin
    ipe_parent = (ipe1_parent & {4{ipe_id[0]}}) | (ipe2_parent & {4{ipe_id[1]}}) | (ipe3_parent & {4{ipe_id[2]}}) | (ipe4_parent & {4{ipe_id[3]}}) ;
  end
  endfunction
`endif

`ifndef OMIT_IPE_FIXES
  wire      [3:0] pc_in_ipe = address_in_ipe (buff_pc);
`else
  wire      [3:0] pc_in_ipe = address_in_ipe (fe_pc);
`endif

wire    [3:0] eu_mem_in_ipe = address_in_ipe (eu_mab);
wire       [3:0] dma_in_ipe = address_in_ipe (dma_addr);
wire       [3:0] dbg_in_ipe = address_in_ipe (dbg_mem_addr);
...
`ifndef OMIT_IPE_FIXES
  ...
  `ifdef IPE_GRAN
    ...
    wire [15:0] entry_point3 = (MPUIP3SEGB1 << shift_amt[2])+ 8;
    wire [15:0] entry_point4 = (MPUIP4SEGB1 << shift_amt[3])+ 8;
  `else
    wire [15:0] entry_point = (MPUIPSEGB1 << 4) + 8;
    wire [15:0] entry_point2 = (MPUIP2SEGB1 << 4) + 8;
    wire [15:0] entry_point3 = (MPUIP3SEGB1 << 4) + 8;
    wire [15:0] entry_point4 = (MPUIP4SEGB1 << 4) + 8;
  `endif
  wire       [3:0] fe_pc_in_ipe = address_in_ipe (fe_pc);
  wire   [3:0] fe_pc_nxt_in_ipe = address_in_ipe (fe_pc_nxt);
  wire    [3:0] fe_nxt_in_entry = {(fe_pc_nxt == entry_point4), (fe_pc_nxt == entry_point3) ,(fe_pc_nxt == entry_point2), (fe_pc_nxt == entry_point)};
...
always @(posedge mclk or posedge puc_rst)
  ...
  else if (|pc_in_ipe) IPE_ACTIVE[3:0] <= pc_in_ipe;
...
wire [15:0] ip3c0_rd  = MPUIP3C0  & {16{reg_rd[IPC0_OFFSET]}} & {16{reg_sel3}};
wire [15:0] seg3b2_rd  = MPUIP3SEGB2  & {16{reg_rd[IPSEGB2_OFFSET]}} & {16{reg_sel3}};
wire [15:0] seg3b1_rd  = MPUIP3SEGB1  & {16{reg_rd[IPSEGB1_OFFSET]}} & {16{reg_sel3}};
wire [15:0] ip4c0_rd  = MPUIP4C0  & {16{reg_rd[IPC0_OFFSET]}} & {16{reg_sel4}};
wire [15:0] seg4b2_rd  = MPUIP4SEGB2  & {16{reg_rd[IPSEGB2_OFFSET]}} & {16{reg_sel4}};
wire [15:0] seg4b1_rd  = MPUIP4SEGB1  & {16{reg_rd[IPSEGB1_OFFSET]}} & {16{reg_sel4}};
wire [15:0] active_rd  = IPE_ACTIVE  & {16{reg_rd[IPE_ACTIVE_OFFSET]}};

assign per_dout        =  (ipc0_rd | ip2c0_rd | ip3c0_rd | ip4c0_rd) |
                          (segb2_rd | seg2b2_rd | seg3b2_rd | seg4b2_rd) |
                          (segb1_rd | seg2b1_rd | seg3b1_rd | seg4b1_rd) |
                          active_rd;
