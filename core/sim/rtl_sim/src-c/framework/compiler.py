...
class ArgumentRegCounter(c_ast.NodeVisitor):
    ...
    def nb_reg_used(argTypes):
        ...
        for argType in argTypes:
            if argType in ["long long", "signed long long", "unsigned long long", "double", "long double", "uint64_t", "int64_t"]:  # 64 bit == 4 reg
                nb_reg += 4
            elif argType in ["long", "signed long", "unsigned long", "float", "uint32_t", "int32_t"]:  # 32 bit == 2 reg
                nb_reg += 2
