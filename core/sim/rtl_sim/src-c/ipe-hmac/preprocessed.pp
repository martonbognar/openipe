# 1 "ipe.c"
# 1 "/openipe/core/sim/rtl_sim/src-c/ipe-hmac//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "ipe.c"
# 1 "mock.h" 1
...
# 2 "ipe.c" 2
# 1 "../framework/libipe/ipe_support.h" 1
...
typedef long long int64_t;
typedef unsigned long long uint64_t;
# 24 "../framework/libipe/ipe_support.h"
typedef struct
{
    uint16_t ipc0 ;
    uint8_t *ipb2 ;
...
# 61 "../framework/libipe/ipe_support.h"
int outside_IPE_segment(char* ptr);
int constant_time_cmp(const unsigned char *x_, const unsigned char *y_, const unsigned int n);
# 71 "../framework/libipe/ipe_support.h"
void __attribute__((weak)) __attribute__((section (".ipe_func"))) *__ipe_memset(void *ptr, int value, int num);
...
ipe_init_struct_t __attribute__((section (".ipestruct"))) ipe_init_struct = { .ipc0 = 0x0080 | 0x0040, .ipb2 = &__ipe_end_shift, .ipb1 = &__ipe_start_shift, .check = (uint16_t) &__ipe_checksum, }; ipe_meta_struct_t __attribute__((section (".ipe_meta"))) ipe_meta_struct = { .ipe_sig_valid = 0xAAAA, .ipe_struct_ptr_src = (uint16_t) &__ipe_struct_shift, };;
...
# 157 "ipe.c"
static inline __attribute__((always_inline)) void private_memcpy(void *dest, const void *source, uint32_t size) {
    uint8_t *dst = dest;
...
static inline __attribute__((always_inline)) uint32_t load32(uint8_t *b) {
  uint32_t x;
  private_memcpy(&x, b, 4);
  return x;
}

static inline __attribute__((always_inline)) void store32(uint8_t *b, uint32_t i) { private_memcpy(b, &i, 4); }

static inline __attribute__((always_inline)) void store64(uint8_t *b, uint64_t i) { private_memcpy(b, &i, 8); }
# 197 "ipe.c"
...
static void __attribute__((section (".ipe_func")))
Hacl_Hash_Lib_LoadStore_uint32s_from_be_bytes(uint32_t *output, uint8_t *input, uint32_t len)
{
  for (uint32_t i = (uint32_t)0U; i < len; i = i + (uint32_t)1U)
  {
    ...
    uint32_t inputi = (((({ uint32_t _temp = ((load32(x0))); ((_temp >> 24) & 0x000000FF) | ((_temp >> 8) & 0x0000FF00) | ((_temp << 8) & 0x00FF0000) | ((_temp << 24) & 0xFF000000); }))));
    output[i] = inputi;
  }
}
...
Hacl_Hash_Lib_LoadStore_uint32s_to_be_bytes(uint8_t *output, uint32_t *input, uint32_t len)
{
  for (uint32_t i = (uint32_t)0U; i < len; i = i + (uint32_t)1U)
  {
    ...
    (store32(x0, (({ uint32_t _temp = (hd1); ((_temp >> 24) & 0x000000FF) | ((_temp >> 8) & 0x0000FF00) | ((_temp << 8) & 0x00FF0000) | ((_temp << 24) & 0xFF000000); }))));
  }
}

static void __attribute__((section (".ipe_func"))) Hacl_Impl_SHA2_256_init(uint32_t *state)
{
  uint32_t *k1 = state;
...
static void __attribute__((section (".ipe_func"))) Hacl_Impl_SHA2_256_update(uint32_t *state, uint8_t *data)
{
...
static void __attribute__((section (".ipe_func"))) Hacl_Impl_SHA2_256_update_multi(uint32_t *state, uint8_t *data, uint32_t n1)
{
...
static void __attribute__((section (".ipe_func"))) Hacl_Impl_SHA2_256_update_last(uint32_t *state, uint8_t *data, uint32_t len)
{
  ...
  (store64(buf2, (({ uint64_t __temp = (encodedlen); uint32_t __low = (({ uint32_t _temp = ((uint32_t)__temp); ((_temp >> 24) & 0x000000FF) | ((_temp >> 8) & 0x0000FF00) | ((_temp << 8) & 0x00FF0000) | ((_temp << 24) & 0xFF000000); })); uint32_t __high = (({ uint32_t _temp = ((uint32_t)(__temp >> 32)); ((_temp >> 24) & 0x000000FF) | ((_temp >> 8) & 0x0000FF00) | ((_temp << 8) & 0x00FF0000) | ((_temp << 24) & 0xFF000000); })); (((uint64_t)__low) << 32) | __high; }))));
  Hacl_Impl_SHA2_256_update_multi(state, final_blocks, nb);
}

static void __attribute__((section (".ipe_func"))) Hacl_Impl_SHA2_256_finish(uint32_t *state, uint8_t *hash1)
{
  uint32_t *hash_w = state + (uint32_t)128U;
  Hacl_Hash_Lib_LoadStore_uint32s_to_be_bytes(hash1, hash_w, (uint32_t)8U);
}

static void __attribute__((section (".ipe_func"))) Hacl_Impl_SHA2_256_hash(uint8_t *hash1, uint8_t *input, uint32_t len)
{
...
static void __attribute__((section (".ipe_func"))) Hacl_Impl_HMAC_SHA2_256_xor_bytes_inplace(uint8_t *a, uint8_t *b, uint32_t len)
{
  for (uint32_t i = (uint32_t)0U; i < len; i = i + (uint32_t)1U)
  {
    uint8_t xi = a[i];
...
void __attribute__((section (".ipe_func"))) hmac_core(uint8_t *mac, uint8_t *key, uint8_t *data, uint32_t len)
{
  Hacl_Impl_HMAC_SHA2_256_hmac_core(mac, key, data, len);
}

void __attribute__((section (".ipe_func"))) hmac(uint8_t *mac, uint8_t *key, uint32_t keylen, uint8_t *data, uint32_t datalen)
{
  Hacl_Impl_HMAC_SHA2_256_hmac(mac, key, keylen, data, datalen);
}

uint8_t __attribute__((section (".ipe_const"))) persistent_key[65] = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";





int __attribute__((section (".ipe_entry"))) attest(void)
{
  uint8_t key[64] = {0};
  for (int i = 0; i < 64; ++ i)
  {
